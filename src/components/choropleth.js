...
function ChoroplethMap(props) {
  ...
  const mapData = (selector) => {
    ...
    function ready([canada]) {
      ...
      console.log(topojson.feature(canada, canada.objects.canada).features)
      svg.append('g')
          .attr('class', 'states')
          .selectAll('path')
          ...
          .append('title')
          .text(function(d) {
            console.log(">>" + unemployment.get(d.properties.ST_NM.toLowerCase()))
      ...
      svg.append('path')
          ...
          .attr('fill', 'none')
          .attr('stroke-width', 2)
          .attr('d', path(topojson.mesh(canada, canada.objects.canada)));
    };
  };
  ...
  return (
    <div className="ChoroplethMap fadeInUp" style={{animationDelay: '1.2s'}}>
      <h1 className="header">Statistics by Province</h1>
      <h6 className="header">{window.innerWidth <=769 ? 'Tap' : 'Hover'} over a state for more details</h6>
      <div className="svg-parent">
        <svg id="chart" width="800" height={window.innerWidth <= 479 ? 650: 750} viewBox={`0 0 900 ${window.innerWidth <= 479 ? 650: 750}`} preserveAspectRatio="xMidYMid meet" ref={choroplethMap}></svg>
      </div>
